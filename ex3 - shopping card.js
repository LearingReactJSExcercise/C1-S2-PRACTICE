...
function getCartTotalAmount() {
  ...
  for ( let cart of SHOPPING_CART){
      let product = PRODUCTS.find((product) => product.id === cart.id);
      if (product){
        result += product.price * cart.quantity;
      }
  }
  return result;
...
function addProductToCart(productId) {
  // Write your code here
  let existProduct = SHOPPING_CART.find((cart)=> cart.id === productId);
  if (existProduct){
  ...
  if (existProduct){
    if (existProduct.quantity >=2){
      existProduct.quantity--;
        ...
        let index = SHOPPING_CART.indexOf(existProduct);
